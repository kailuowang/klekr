...
describe('PicturePreloadPriority', function() {
  ...
  // Define the PicturePreloadPriority class for testing
  const PicturePreloadPriority = class PicturePreloadPriority {
  ...
  describe('priority for', function() {
    ...
    it('small higher than full for the same picture', function() {
      [true, false].forEach(inGrid => {
    ...
    it('small higher than full in the same page when in grid', function() {
      gallery._inGrid = true;
      for (let p = 0; p <= 4; p++) {
        const pageStart = p * 10;
        const pageEnd = p * 10 + 9;
    ...
    it('small in next page higher than full in current page when in grid', function() {
      gallery._inGrid = true;
    ...
    it('full for next 6 pictures higher than all other smalls except the next 6 pictures when not in grid', function() {
    ...
    it('full in current page higher than small in more than 1 page away', function() {
      [true, false].forEach(inGrid => {
        gallery._inGrid = inGrid;
        for (let currentPage = 10; currentPage <= 19; currentPage++) {
    ...
    it('full within the next pagesize decrease as distance from the current', function() {
      [true, false].forEach(inGrid => {
        gallery._inGrid = inGrid;
    ...
    it('full ahead of current higher than full before current in the same page', function() {
      [true, false].forEach(inGrid => {
        gallery._inGrid = inGrid;
    ...
    it('anything in previous pages is lower than anything in current page forward', function() {
      [true, false].forEach(inGrid => {
        gallery._inGrid = inGrid;
    ...
    it('full within the next pagesize decrease as distance from the current (duplicate)', function() {
      [true, false].forEach(inGrid => {
        gallery._inGrid = inGrid;
        for (let withinNextPage = 12; withinNextPage <= 22; withinNextPage++) {
          for (let restAhead = withinNextPage + 1; restAhead <= 49; restAhead++) {
