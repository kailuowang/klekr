...
(function() {
  ...
  window.MySourcesView = (function(superClass) {
    ...
    function MySourcesView() {
      ...
      this._loadMoreSources = bind(this._loadMoreSources, this);
      this._checkIfMoreContentNeeded = bind(this._checkIfMoreContentNeeded, this);
      this._setupInfiniteScroll = bind(this._setupInfiniteScroll, this);
    ...
    MySourcesView.prototype.onAllSourcesLoaded = function(empty) {
      ...
      if (!empty) {
        ...
        return this._setupInfiniteScroll();
      }
    };
    ...
    MySourcesView.prototype._setupInfiniteScroll = function() {
      console.log("Setting up infinite scroll");
      this.currentPage = 1;
      this.isLoading = false;
      this.hasMoreSources = true;
      $(window).off('scroll.mySources');
      $(window).on('scroll.mySources', (function(_this) {
        return function() {
          return _this._checkIfMoreContentNeeded();
        };
      })(this));
    ...
    MySourcesView.prototype._checkIfMoreContentNeeded = function() {
      var documentHeight, scrollPosition;
      scrollPosition = $(window).scrollTop() + $(window).height();
      documentHeight = $(document).height();
      if (scrollPosition >= documentHeight - 200) {
        console.log("Near bottom, loading more if possible");
        if (!(this.isLoading || !this.hasMoreSources)) {
          this._loadMoreSources();
        }
      }
      if (documentHeight <= $(window).height() && this.hasMoreSources && !this.isLoading) {
        console.log("No scrollbar yet, loading more content");
        return this._loadMoreSources();
      }
    };

    MySourcesView.prototype._loadMoreSources = function() {
      this.isLoading = true;
      this.currentPage += 1;
      this.indicator.show();
      ...
      return klekr.Global.server.get(my_sources_flickr_streams_path(), {
        ...
        return function(data) {
          var d, sources;
          console.log("Page", _this.currentPage, "loaded with", data.length, "items");
          sources = (function() {
            var j, len, results;
            results = [];
          ...
          if (sources.length > 0) {
            ...
            _this._registerCellEvents();
            setTimeout(function() {
              return _this._checkIfMoreContentNeeded();
            }, 100);
          } else {
            _this.hasMoreSources = false;
            console.log("No more sources to load");
          }
          _this.isLoading = false;
          return _this.indicator.hide();
        };
      })(this));
    };

    MySourcesView.prototype._display = function(sources) {
      var j, len, results, source;
      console.log("Displaying", sources.length, "sources");
      results = [];
      for (j = 0, len = sources.length; j < len; j++) {
        source = sources[j];
        results.push(this.addSource(source));
      }
      return results;
    };
...
(function() {
  ...
  window.MySources = MySources = (function() {
    function MySources() {
