module Collectr
  module SlideshowControllerHelper
    ...
    def slideshow_for_stream(stream)
      ...
      # Set flag to skip loading stream info since we already know the stream
      # Removing this flag as it might be causing issues
      # @skip_stream_info = true
    ...
    def check_stream_access(stream)
      if(current_collector && stream.collector != current_collector)
        redirect_to user_path(id: stream.user_id, type: stream.class.name)
      end
    end
